...
import { Trophy, Award, Crown, Medal, ShieldCheck, Download, Share2, X, ChevronDown, Users, Flame } from 'lucide-react';
...
export default function LeaderboardPodium() {
  ...
  const handleShowMore = async () => {
    if (!activeChallenge || !activeChallenge.id || isLoadingMore) return;
    setIsLoadingMore(true);
    const nextSkip = skip + 50;
    try {
      const data = await api.getChallengeLeaderboard(activeChallenge.id, nextSkip, 50);
      if (data && data.items && data.items.length > 0) {
        setParticipants(prev => [...prev, ...data.items]);
        setSkip(nextSkip);
        setHasMore((participants.length + data.items.length) < data.total_count);
      } else {
        setHasMore(false);
      }
    } catch {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 pb-28 animate-in fade-in duration-300">
      ...
      {/* ── Top 3 Grand Podium / Empty State ── */}
      {participants.length >= 3 ? (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-end pt-8">
          ...
          <div className="order-1 sm:order-2 p-8 rounded-3xl bg-gradient-to-b from-[#FFFDF9] to-[#FBF4E4] dark:from-[#1E1912] dark:to-[#121620] border-2 border-[#C5A059] shadow-2xl text-center space-y-4 flex flex-col items-center relative -translate-y-4">
            ...
            >
              <Trophy size={16} />
              <span>Rasmiy Oltin Sertifikatni Ochish</span>
            </button>
          </div>
          ...
          <div className="order-3 p-6 rounded-3xl bg-white dark:bg-[#121620] border border-stone-200/90 dark:border-white/10 shadow-xs text-center space-y-3 flex flex-col items-center">
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-stone-100 dark:bg-white/10 text-xs font-mono font-bold hover:bg-[#E05638] hover:text-white transition-colors cursor-pointer"
            >
              Bronza Sertifikatni Ochish →
            </button>
          </div>

        </div>
      ) : (
        <div className="p-8 sm:p-14 rounded-3xl bg-white dark:bg-[#121620] border border-stone-200/90 dark:border-white/10 text-center space-y-4 max-w-2xl mx-auto shadow-xs">
          ...
          <div className="space-y-2">
            <h3 className="font-serif text-2xl font-bold text-stone-900 dark:text-white">
              Yangi Chempionat Boshlandi
            </h3>
            <p className="text-xs sm:text-sm text-stone-500 leading-relaxed max-w-md mx-auto">
              Hozircha g'oliblar shohsupasi bo'sh. Saytda kitob mutolaa qiling, soatlar to'plang va 1-o'rinni egallab rasmiy sertifikat sohibi bo'ling!
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-[#121620] border border-stone-200/90 dark:border-white/10 rounded-3xl overflow-hidden shadow-xs">
        ...
        <div className="p-6 sm:p-8 border-b border-stone-100 dark:border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-stone-500 mt-0.5">
              Haqiqiy o'qish tezligi va sinovlaridan muvaffaqiyatli o'tgan reyting
            </p>
          </div>
        ...
        {participants.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    {avatarUrl ? (
                      ...
                      <div className="w-10 h-10 rounded-full bg-[#E05638] text-white flex items-center justify-center font-bold text-sm shrink-0">
                        {uName[0]}
                      </div>
                    )}
          ...
          <div className="p-10 text-center text-stone-400 font-mono text-xs">
            Hozircha chempionat ishtirokchilari mavjud emas. Yangi asarlarni o'qib, reytingda birinchi o'rinni egallang!
          </div>
        )}
        ...
        {hasMore && (
          <div className="p-6 border-t border-stone-100 dark:border-white/5 text-center bg-stone-50/50 dark:bg-white/[0.01]">
            ...
            >
              ...
              <span>{isLoadingMore ? "Yuklanmoqda..." : "Yana 50 nafar kitobxonni ko'rish"}</span>
            </button>
          </div>
        )}
