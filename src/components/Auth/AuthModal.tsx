...
export default function AuthModal({ onSuccess, onCancel }: Props) {
  ...
  useEffect(() => {
    ...
    // Mount Telegram widget script
    const container = document.getElementById('telegram-widget-box');
    if (container) {
      container.innerHTML = '';
      const s = document.createElement('script');
      s.src = 'https://telegram.org/js/telegram-widget.js?22';
      s.setAttribute('data-telegram-login', 'book1fy_bot');
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2.5">
              ...
              <div className="relative w-full">
                <div id="telegram-widget-box" className="w-full flex justify-center empty:hidden" />
                <button
                  type="button"
                  onClick={handleTelegramClick}
                  className="w-full py-3 px-4 rounded-full bg-[#24A1DE] hover:bg-[#208fca] active:scale-[0.98] text-white font-medium text-xs flex items-center justify-center gap-3 transition-all shadow-md hover:shadow-lg cursor-pointer"
                >
