...
export default function BookPaywallModal({ book, onClose, onAccessGranted, isAdmin }: PaywallModalProps) {
  ...
  const handlePay = async (planType: 'book' | 'vip_monthly' | 'vip_yearly') => {
    ...
    try {
      ...
      if (planType === 'book') payload.book_id = book.id;
      ...
      >
        ...
        >
          ...
          <div className="px-6 pt-6 pb-4 text-center relative">
            ...
            <button
              onClick={onClose}
              className="absolute right-4 top-4 w-8 h-8 rounded-full flex items-center justify-center text-white/50 hover:text-white hover:bg-white/10 transition-all"
              aria-label="Yopish"
            >
              ✕
            </button>
          ...
          <div className="px-6 py-4">
            {tab === 'book' ? (
              ...
              <div>
                <p className="text-white/70 text-sm text-center mb-4">
                  Faqat shu kitobni bir martalik to'lov bilan oching
                </p>
                ...
                >
                  ...
                  <div className="mt-4 space-y-2 text-left">
                    {['📖 Butun kitobni o\'qish', '🔖 Sahifa belgisi qo\'yish', '🎧 Audio versiyasi (mavjud bo\'lsa)', '🔁 Istalgan vaqt qayta kirish'].map(perk => (
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full py-4 rounded-2xl font-bold text-base text-black transition-all active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
                  style={{ background: loading ? '#999' : 'linear-gradient(135deg, #f7971e, #ffd200)', boxShadow: loading ? 'none' : '0 8px 24px rgba(247,151,30,0.4)' }}
                >
                  {loading ? '⏳ Yuklanmoqda...' : `💳 ${formatPrice(book.price || 0)} to'lash`}
                </button>
              </div>
            ) : (
              /* ── VIP Plans ── */
              <div>
                <p className="text-white/70 text-sm text-center mb-4">
                  Barcha premium kitoblarga cheksiz kirish 👑
                </p>
            ...
            {error && (
              <div
                className="mt-3 px-4 py-3 rounded-xl text-sm text-red-300 text-center"
