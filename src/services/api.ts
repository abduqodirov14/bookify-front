...
export const api = {
  ...
  // Telegram Admin & Moderation API (Web Admin Only)
  async getTelegramStats() {
    const token = getAuthToken();
    const res = await fetch(`${API_BASE_URL}/admin/telegram/stats`, {
  ...
  async getTelegramAnnualCharts() {
    const token = getAuthToken();
    const res = await fetch(`${API_BASE_URL}/admin/telegram/annual-charts`, {
      headers: { ...(token ? { 'Authorization': `Bearer ${token}` } : {}) }
    });
    if (!res.ok) return null;
    return res.json();
  ...
  async getTelegramUsers(search?: string, filterStatus?: string) {
    const token = getAuthToken();
    const params = new URLSearchParams();
    if (search) params.append('search', search);
    if (filterStatus && filterStatus !== 'all') params.append('filter_status', filterStatus);

    const res = await fetch(`${API_BASE_URL}/admin/telegram/users?${params.toString()}`, {
      headers: { ...(token ? { 'Authorization': `Bearer ${token}` } : {}) }
    });
    if (!res.ok) return [];
    return res.json();
  ...
  async toggleTelegramBan(telegramId: string, reason?: string) {
    const token = getAuthToken();
    const res = await fetch(`${API_BASE_URL}/admin/telegram/users/${telegramId}/ban`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(token ? { 'Authorization': `Bearer ${token}` } : {})
      },
      body: JSON.stringify({ reason: reason || "Admin tomonidan bloklandi" })
    });
    if (!res.ok) throw new Error("Ban holatini o'zgartirishda xatolik");
    return res.json();
  },

  async toggleTelegramMute(telegramId: string) {
    const token = getAuthToken();
    const res = await fetch(`${API_BASE_URL}/admin/telegram/users/${telegramId}/mute`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(token ? { 'Authorization': `Bearer ${token}` } : {})
    ...
    if (!res.ok) throw new Error("Mute holatini o'zgartirishda xatolik");
    return res.json();
  },

  async sendTelegramBroadcast(data: { title: string; message: string; image_url?: string; button_text?: string; button_url?: string }) {
    const token = getAuthToken();
    const res = await fetch(`${API_BASE_URL}/admin/telegram/broadcast`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(token ? { 'Authorization': `Bearer ${token}` } : {})
    ...
    if (!res.ok) throw new Error("Xabarnoma yuborishda xatolik");
    return res.json();
  },
