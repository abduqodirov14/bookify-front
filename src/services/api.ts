...
// Resilient Fetch with Auto-Retry (absorbs Render/AWS cold-start delays)
...
export const api = {
  ...
  async getBookPages(id: string) {
    try {
      const res = await fetchWithRetry(`${API_BASE_URL}/books/${id}/pages`);
      if (!res.ok) return [];
      return res.json();
    } catch {
      return [];
  ...
  async updateProgress(bookId: string, percent: number, chapterId?: string, readingSeconds: number = 0) {
    return this.syncProgress(bookId, percent, chapterId, readingSeconds);
  },
